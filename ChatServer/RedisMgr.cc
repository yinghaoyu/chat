...
RedisMgr::RedisMgr()
{
    ...
    _con_pool.reset(
        new RedisConPool(10, host.c_str(), atoi(port.c_str()), pwd.c_str()));
}
...
bool RedisMgr::Set(const std::string& key, const std::string& value)
{
    ...
    auto reply = (redisReply*)redisCommand(
        connect, "SET %s %s", key.c_str(), value.c_str());
...
bool RedisMgr::LPush(const std::string& key, const std::string& value)
{
    ...
    auto reply = (redisReply*)redisCommand(
        connect, "LPUSH %s %s", key.c_str(), value.c_str());
    if (NULL == reply)
    {
...
bool RedisMgr::RPush(const std::string& key, const std::string& value)
{
    ...
    auto reply = (redisReply*)redisCommand(
        connect, "RPUSH %s %s", key.c_str(), value.c_str());
    if (NULL == reply)
    {
...
bool RedisMgr::HDel(const std::string& key, const std::string& field)
{
    ...
    redisReply* reply = (redisReply*)redisCommand(
        connect, "HDEL %s %s", key.c_str(), field.c_str());
    if (reply == nullptr)
    {
